...
export function replayPhase03a({ root, psql, applySqlFile, comparator, adaptations, pgTapSql }) {
  ...
  if (!pgTapSql) throw new Error('Phase03A requires the explicitly pinned --phase03a-pgtap-sql file');
  if (hash(fs.readFileSync(pgTapSql)) !== contract.localPgTap.generatedSqlSha256) {
  ...
  const result = {
    ...
    scope: 'Six local SQL candidates; FDA012 managed-role residual and FDA028 excluded from acceptance',
    beforeSupplement, localBaselineSupplement: contract.localBaselineSupplement, baselineAclVerification,
  ...
  result.localProofPassed = result.pgTap.suites.every(s => s.passed) && result.restorationExact && result.reapplyDeterministic;
  result.status = result.localProofPassed ? 'LOCAL_CANDIDATE_PROOF_PASS' : 'LOCAL_CANDIDATE_PROOF_FAIL';
