...
export async function loadRecentlyViewed(userId: string): Promise<string[]> {
  try {
    ...
    console.warn('[recentlyViewed] load failed:', errorMessage(e, 'AsyncStorage error.'));
    return [];
  }
}
 ...
 */
export async function recordView(userId: string, flagId: string): Promise<void> {
  try {
    ...
    console.warn('[recentlyViewed] record failed:', errorMessage(e, 'AsyncStorage error.'));
  }
}
...
export async function clearRecentlyViewed(userId: string): Promise<void> {
  try {
    ...
    console.warn('[recentlyViewed] clear failed:', errorMessage(e, 'AsyncStorage error.'));
  }
}
 ...
 */
export async function dropFromRecent(userId: string, flagId: string): Promise<void> {
  try {
    const existing = await loadRecentlyViewed(userId);
    ...
    console.warn('[recentlyViewed] drop failed:', errorMessage(e, 'AsyncStorage error.'));
  }
}
