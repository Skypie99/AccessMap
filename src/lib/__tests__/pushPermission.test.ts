...
// ── expo-notifications virtual mock ─────────────────────────────────────────
// The helpers `require('expo-notifications')` dynamically; `virtual: true`
// lets us mock a module that isn't physically installed. Each test sets the
// status these fns resolve to. Names are `mock`-prefixed because jest hoists
// jest.mock() above imports and forbids referencing non-`mock` outer vars.
