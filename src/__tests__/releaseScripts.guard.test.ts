...
describe('Release R1 D8 fail-closed gate', () => {
  it('defaults to no and requires exact yes before dependency installation', () => {
    expect(dispatchInputs).toMatch(
      /d8_closed:\n\s+description:.*\n\s+required: false\n\s+default: 'no'\n\s+type: string/,
    );

    const d8Gate = stepBlock('name: Require D8 final-device proof');
    expect(d8Gate).toContain('D8_CLOSED: ${{ inputs.d8_closed }}');
    expect(d8Gate).toContain('"$D8_CLOSED" != "yes"');
    expect(d8Gate).not.toMatch(/\bprofile\b/i);
    expect(sourceIndex('name: Require D8 final-device proof')).toBeLessThan(
      sourceIndex('run: npm ci --legacy-peer-deps'),
    );
  });
});
...
describe('Release R1 approval, permissions, and release semantics', () => {
  ...
  it('creates no GitHub Release and writes only a non-secret outcome receipt', () => {
    ...
    expect(summary).toContain('EAS build/submit step outcome');
