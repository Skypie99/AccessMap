...
describe('T4 — TasksScreen triage speaks the commit vocabulary (source contract)', () => {
  ...
  // Phase 03B split prompt/gating from the actual atomic status commit. Pin the
  // commit function so the outcome haptic and failure handling remain guarded
  // without requiring the reason-picker wrapper to duplicate them.
  const commitStatusStart = tasksScreen.indexOf('const commitStatus');
  const commitStatusEnd = tasksScreen.indexOf('const setStatus', commitStatusStart);
  const commitStatus = tasksScreen.slice(commitStatusStart, commitStatusEnd);
  ...
  it('the slice really is commitStatus — a bad window would pass vacuously', () => {
    // Both boundaries must have been found, or `slice` silently yields
    // something that still satisfies a positive match by accident.
    expect(commitStatusStart).toBeGreaterThan(-1);
    expect(commitStatusEnd).toBeGreaterThan(commitStatusStart);
    expect(commitStatus).toMatch(/updateFlagStatus\(/);
  });

  it('the failure path fires notify(error) in the commitStatus catch', () => {
    expect(commitStatus).toMatch(/catch \(e\) \{[\s\S]*?hapticNotify\('error'\)/);
  });
