...
> Required for any change touching: location data, disability/flag data, user identity, auth flows, photo uploads, or new analytics.
...
- [ ] No new synchronous blocking on the JS thread (heavy work in `useEffect` or worker)
...
<!-- Steps needed after merge: SQL migrations to apply, env vars to add, feature flags to toggle, etc. If none, write "None." -->
