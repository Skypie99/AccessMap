...
prereqs AS (
  SELECT
    count(*) FILTER (WHERE name = 'fda028_limiter_epoch_key')::integer AS mf03_rows,
    COALESCE(bool_and(
      decrypted_secret ~ '^[0-9A-Fa-f]{64}$'
      AND octet_length(decode(decrypted_secret, 'hex')) = 32
    ) FILTER (WHERE name = 'fda028_limiter_epoch_key'), false) AS mf03_shape_valid,
    count(*) FILTER (WHERE name = 'webhook_endpoint')::integer AS mf04_rows,
    COALESCE(bool_and(
      decrypted_secret ~ '^https://[a-z0-9.-]+(:[0-9]+)?(/|$)'
    ) FILTER (WHERE name = 'webhook_endpoint'), false) AS mf04_shape_valid,
    count(*) FILTER (WHERE name = 'webhook_secret')::integer AS webhook_secret_rows,
    COALESCE(bool_and(length(decrypted_secret) > 0)
      FILTER (WHERE name = 'webhook_secret'), false) AS webhook_secret_nonempty
...
functions AS (
  SELECT n.nspname AS schema_name,
    ...
    p.proacl AS acl,
    encode(extensions.digest(pg_get_functiondef(p.oid)::bytea, 'sha256'), 'hex') AS definition_sha256,
