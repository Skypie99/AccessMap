...
3. **Identifiers in `eas.json` (committed to git).** The file inlines your `appleId` ([REDACTED_EMAIL]), `appleTeamId` (S78F8ZA8QU), and now `ascAppId` (6774709116). These are identifiers, not secrets, and were already inlined by the existing file — I matched that convention. One transient version of the file during my run had them scrubbed to placeholders + env-var instructions. **If you'd rather keep your Apple ID email out of git, say so** and I'll switch the submit block to `EXPO_APPLE_ID` / `EXPO_APPLE_TEAM_ID` env vars + EAS secrets. No real secret (token/password/key) is in the file either way.
