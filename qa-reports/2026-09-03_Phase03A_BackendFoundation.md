...
## Owner-resume addendum — FDA-012 local proof and confirmed FDA-028 hold

This append-only 2026-09-05 UTC receipt supersedes only earlier current-state interpretations. The complete preceding 31,677 bytes remain identical, SHA-256 `d49a6fb0f88ae3995481f6149e4d58af161694efcbb5ca06415faf4f07e63ffa`. The original preflight and first-resume decisions/results remain historical evidence.

**BACKEND_FOUNDATION_GATE: BLOCKED. PHASE_GATE: BLOCKED. SAFE_TO_INTEGRATE: NO. MAIN_MERGE_AUTHORIZED: NO.**

Prompt: `FLAGSTONE PHASE-03A — BACKEND HARDENING FOUNDATION`; repository remote `Skypie99/AccessMap`. Authority is the latest owner-resume attachment plus Sky's explicit reply: **“Keep the budget across session resets; retain the architecture hold until a trusted mechanism is approved.”** This is a blocked handoff, not accepted Phase 03A or a production-authorization packet.

Accepted predecessor remains `c2e36800b269ee22f29d0be35cfb88dace7c2afc` / `7a68541462f0a9e1d55f48d98ea54df0fc0b01b7`. Resume began clean at `c794ba8544df2f128269eabf6d87c9ed07a8fd68` / `9f063b465f11127a590d1863d6ebf7a63abf8e55`. New unaccepted partial source is `0a6a6b03fd0cbe72f70f67260f6cab746e098f6a` / `857411dc733b93686d789a72e461856698bec814`, branch `repair/flagstone-p03a-backend-foundation-20260903`, worktree `/Users/skypie/AccessMap-codex/p03a-backend-foundation-20260903`. The independent reviewer verified that exact source clean. A following documentation-only commit packages this addendum; its SHA/tree and final clean status are resolved in the exported finalization receipt without changing reviewed source.

FDA-012 now has a seventh CLI-generated application-privilege migration and matching exact restoration. The six earlier pairs are byte-identical. Effective table and independent column rights, PUBLIC/inherited access, grant options, owners, RLS/invoker behavior, schemas, role membership and future postgres defaults are enforced by an exhaustive allowlist and actual database guard. CI and replay reject unlisted/missing/drifted artifacts before application. Narrow deployed service capabilities are preserved from saved source evidence, not inferred from the unshipped Phase 04 implementation. The source increment changes 20 files; all 39 non-QA files changed from accepted Phase 02 and every pair/caller hash are recorded in the frozen-source evidence.

Supabase-managed defaults are a documented residual under the owner's decision: all 48 managed ACL entries are captured; 36 client/service entries are modeled locally, with the other 12 owner-grantee entries explicitly outside that model. Managed defaults were not changed. The claim is bounded to Flagstone-owned application surfaces. The earlier inability to modify supabase_admin defaults no longer independently blocks FDA-012; unsafe effective app access would still block it.

Independent reviewer `/root/03a_effective_privilege_acceptance` issued **FDA012_LOCAL_SUBSET_ACCEPTANCE: PASS; COMPLETE_PHASE_03A_CODE_GATE: HOLD** with no actionable subset defect. Actual clean-source replay passed **217/217 genuine pgTAP assertions** (25 media, 113 foundation, 79 privileges), **18/18 real database negative mutations**, **8/8 inventory/hash refusals plus a positive control**, **9/9 parser tests**, and schema provenance for 87 inputs. All seven restorations in reverse order recover the exact combined baseline and deterministically reapply. No per-pair intermediate catalog snapshots were captured.

Writer typecheck and lint pass, with 0 errors and 91 inherited warnings. Focused Jest passes 5 suites/182 tests; the changed flags.supabase suite separately passes 40 tests in full Jest. Full Jest remains **273 passed/12 failed suites, 4,204 passed/14 failed/32 TODO tests**, exit 1. All 14 failed names match prior verified evidence exactly; introduced failures: NONE. Earlier REFERENCES fixture failures and incorrect focused-test paths are retained with their corrected final results. Independent tests are distinguished from writer logs inspected by the reviewer. Exact commands, exits, TAP, raw/wrapped logs and byte hashes are packaged with this receipt.

FDA-028 remains unimplemented. The owner requires budgets to survive session resets; no approved trusted continuity/admission primitive currently establishes that while preserving independent same-network clients and the stated privacy limits. IP hashing or freely reissued signed sessions does not satisfy the confirmed contract. Actual hosted metadata/anti-forgery rules remain unverified. No new provider, persistent device identifier, anonymous Auth adoption, gateway probe or limiter was introduced. Existing guest flag/feedback paths remain transitional, with bypass prevention explicitly NOT PROVEN.

The approved staging branch exists at read-only capture: `ctshxbykuemeqnofqcdh`, branch ID `441acc38-d71c-4a87-883e-61ff87e0c52e`, name `flagstone-p03a-staging-20260905`, non-default/nonpersistent development branch of `kldlwszpfkdmsjrjhjym`, ACTIVE_HEALTHY and with_data false. Counts of all 13 public application tables, auth.users, storage.objects and vault.secrets are zero; no rows or values were read. Starting catalog SHA-256 is `1679e494c63483c116b87b8aeac9344aa153d412bb1750b9f205943abbd4f686`. All 71 migration names/versions match; five accepted Phase 02 adoption files are still needed. Baseline is NOT RECONCILED; full staging identity gate is NOT PASSED. This is read-only feasibility, not formal STAGE acceptance. Generic get_project returned NotFoundException; positive branch/SQL evidence is recorded separately.

Production comparator-v3 metadata at `2026-09-05T07:31:07.892Z` and `2026-09-05T08:17:17.017Z` is identical, SHA-256 `6f3ee7f60f4134a83b2e013f4f3ffe98be1ae0fa489032062ee2f38cad9ca0d4`. No production or staging mutation occurred. This verifies bounded catalog metadata, not runtime, delivery or every platform setting. No application rows, credentials or secrets were collected.

All 71 applied sources, accepted Phase 02 candidates, packages/lock, generated schema SQL and previous QA remain preserved. All 41 other registered worktree HEAD/branch/status records are unchanged; 42 total, including the same pre-existing missing path. Main/origin main remain `70b52a30e9fff0f7d538509b110212bb8d872391`, 0/0. Integration worktree remains `5a64c9174ae5d9d5a94ed543bc4663216df67e7f` / `edaa68182add5b9a41123f6864c7774f5969e03f`, with no integration. No new UI changes or visual/accessibility/runtime evidence exists. Current client methods must not ship ahead of their undeployed RPCs. Full app-delete E2E remains PHASE_04_OWNED; only local transitional owner/admin/unauthorized role proof exists.

Restorations remain **UNSAFE_BASELINE_RESTORE**, not safe production rollback authority. Combined baseline/restored hash is `1af751339866c181c600d45135a704b97d0ba1162dc4b318da544f6e75ef45cc`; after/reapplied hash is `372e20ce2ca1afa273cbf5018ad07f22e6c18de898f05bd5e5a5b552b3899795`. Hosted restoration was not run.

**STAGING_CLEANUP_REQUIRED: YES. STAGING_CLEANUP_SAFE: NO. STAGING_BRANCH_DELETED: NO. STAGING_CLEANUP_VERIFIED: NO.** The retained exact branch is `ctshxbykuemeqnofqcdh` / `441acc38-d71c-4a87-883e-61ff87e0c52e`. Sky's explicit no-early-delete rule requires all twelve conditions, including complete CODE/INT/STAGE, independent hosted evidence and the production packet. Those are incomplete. No deletion was attempted. Cleanup is recorded, not forgotten; early deletion would require a separate superseding owner instruction.

## DECISIONS FOR SKY — owner-resume disposition

FDA-028 continuity is decided: retain the architecture hold. Recommendation: approve a specific trusted continuity/admission mechanism only after its privacy/access and reset behavior are concrete. Alternative: explicitly change the contract; that is not currently authorized. Impact: complete CODE, INT and STAGE remain held. If a future design requires a pre-CODE synthetic metadata probe, it needs a concrete source/identity/removal plan and explicit sequencing exception. General staging permission does not change the required order. No routine reapproval is requested. Staging remains subject to the owner's twelve cleanup conditions unless Sky separately authorizes early deletion.

All findings **FDA-009, FDA-010, FDA-012, FDA-021, FDA-023, FDA-026 and FDA-028 remain OPEN**; no inherited or supporting finding/deferral was closed. **03A-CODE: PARTIAL/HOLD; INT: NOT RUN; 03A-STAGE: NOT RUN; 03A-PROD: READ-ONLY PREFLIGHT ONLY.** Hosted pgTAP, REST/RPC, RLS/role matrix, Auth/Storage, limiter isolation/bypass, hosted restoration and independent staging acceptance are NOT RUN. No complete CODE SHA/tree is accepted.

**PRODUCTION_AUTHORITY: NONE. PUSHES: NONE. MERGES: NONE. DEPLOYS: NONE. REMOTE_MUTATIONS: NONE. PHASE_03B_STARTED: NO. PHASE_03B_HANDOFF: NOT ISSUED.** App Store review is protected by no action, with current status unverified. Next permitted work is a scoped Phase 03A resume after approval of the trusted mechanism; STOP before production.

Full report: [2026-09-05_Codex_Phase03AOwnerResume.md](2026-09-05_Codex_Phase03AOwnerResume.md). Evidence: [blocked handoff](phase03a/2026-09-05-owner-resume/blocked-handoff.json), [frozen source and ordered hashes](phase03a/2026-09-05-owner-resume/frozen-source.json), [independent review](phase03a/2026-09-05-owner-resume/independent-acceptance.md), [test results](phase03a/2026-09-05-owner-resume/test-summary.json), [artifact index](phase03a/2026-09-05-owner-resume/artifact-hashes.json). Exact commands and outputs are linked from the full report and independent command receipt.
