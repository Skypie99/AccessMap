...
|----------|--------|---------------|------|-----------------|
| 8 | `jordan/privacy-policy-gaps-2026-05-29` (local) | **NEEDS REVIEW** | 🟡 Medium | 3 commits: EAS CI hardening + privacy policy + incident response docs. **BLOCKER: Branch adds email (`[REDACTED_EMAIL]`) and Apple Team ID (`S78F8ZA8QU`) in plaintext eas.json comments.** Strip PII before merge. Also: EAS CI workflow changes (`.github/workflows/eas-build.yml`, `eas-testflight-submit.yml`) need Steve sign-off on trigger logic. |
