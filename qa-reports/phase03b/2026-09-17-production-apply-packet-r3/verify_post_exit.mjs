...
const entry = entryEnvelope.entry ?? entryEnvelope.phase03b_quiescence_entry_r3 ?? entryEnvelope;
...
const receipt = { result: 'HOLD', target: TARGET, readOnly: true, startedAt: new Date().toISOString(), steps: [] };
try {
  ...
  receipt.steps.push(proofRun.receipt); const proof = row(proofRun.payload, 'phase03b_post_exit_proof_r3');
  ...
  receipt.steps.push(structureRun.receipt); const normalized = normalizeCatalog(row(structureRun.payload, 'catalog'));
  ...
  const fnRun = run('edge-function', ['functions', 'list', '--project-ref', TARGET, '--output-format', 'json']); receipt.steps.push(fnRun.receipt);
  ...
  receipt.finishedAt = new Date().toISOString(); receipt.automaticGateRecreationAuthorized = false;
  writeFileSync(join(evidence, 'POST_EXIT_VERIFIER_RECEIPT.json'), `${JSON.stringify(receipt, null, 2)}\n`, { mode: 0o600, flag: 'wx' });
}
