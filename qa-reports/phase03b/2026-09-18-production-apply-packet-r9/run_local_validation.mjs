...
try {
  ...
  receipt.children.push(await run('LOCAL_R9_CONTROL_VALIDATION', 'validate_r8_controls.mjs'));
  ...
  receipt.preservedR8ReplayCheckCount = r8Checks.length - r8Children.flatMap((child) =>
    Object.entries(child.payload?.r9TransportExactnessCases ?? {})).length;
  receipt.preservedR7ReplayCheckCount = r7Checks.length;
  const branchChild = receipt.children.find((child) => child.label === 'LOCAL_R9_CONTROL_VALIDATION');
