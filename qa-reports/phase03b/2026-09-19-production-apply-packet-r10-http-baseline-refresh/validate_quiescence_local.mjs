...
import { buildEntryEnvelope, classifyGateIdentity, createDeadlines } from './r8_control_lib.mjs';
...
try {
  ...
  const adjudicationSnapshot = JSON.parse(psql(['-qAt', '-f', path.join(PACKET, 'SERVER_STATE_ADJUDICATE.sql')]).split('\n').find((line) => line.startsWith('{')));
