...
begin
  ...
  if (select count(*) from net.http_request_queue)<>0 or (select count(*) from net._http_response)<>6 or v_http_hash<>'db09cd0f61b4405a2540be7541b690df4fa52bd7697c98c1e7e88d37a3f99031' then
    raise exception 'Phase 03B exit refused: HTTP baseline mismatch.' using errcode='P0001';
  end if;
end
$guard$;

select jsonb_build_object('receipt','phase03b_quiescence_pre_exit_r3','captured_at_utc',to_char(pg_catalog.clock_timestamp() at time zone 'UTC','YYYY-MM-DD"T"HH24:MI:SS.US"Z"'),'function_oid',__ENTRY_FUNCTION_OID__,'trigger_oid',__ENTRY_TRIGGER_OID__,'truncate_trigger_oid',__ENTRY_TRUNCATE_TRIGGER_OID__,'flags_count',__ENTRY_FLAGS_COUNT__,'flags_sha256','__ENTRY_FLAGS_SHA256__','history_count',__ENTRY_HISTORY_COUNT__,'history_sha256','__ENTRY_HISTORY_SHA256__') as phase03b_quiescence_pre_exit_r3;
