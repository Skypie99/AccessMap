...
const receipt = { schemaVersion: 1, target: TARGET, result: 'HOLD', startedAt: iso(), readOnly: true, steps: [] };
try {
  ...
  receipt.steps.push(structureStep);
  ...
  const normalized = normalizeCatalog(filtered); const structureSha256 = checksum(normalized);
  ...
  receipt.steps.push(functionStep);
  ...
  receipt.finishedAt = iso(); receipt.quiescenceAutomaticallyExited = false;
  writeJson('POST_APPLY_VERIFIER_RECEIPT.json', receipt);
}
