...
begin
  ...
  if (select count(*) from pg_catalog.pg_proc p join pg_catalog.pg_namespace n on n.oid=p.pronamespace where n.nspname='private' and p.proname='flagstone_phase03b_block_row_lifecycle_r2' and pg_catalog.pg_get_function_identity_arguments(p.oid)='') <> 1
     or (select count(*) from pg_catalog.pg_trigger where tgrelid='public.flags'::pg_catalog.regclass and tgname='aaa_flagstone_phase03b_row_lifecycle_quiescence_r2' and not tgisinternal) <> 1 then
    raise exception 'Phase 03B exit refused: reserved-object cardinality mismatch.' using errcode='P0001';
  end if;
  if 'private.flagstone_phase03b_block_row_lifecycle_r2()'::pg_catalog.regprocedure::oid <> __ENTRY_FUNCTION_OID__
     or (select oid from pg_catalog.pg_trigger where tgrelid='public.flags'::pg_catalog.regclass and tgname='aaa_flagstone_phase03b_row_lifecycle_quiescence_r2' and not tgisinternal) <> __ENTRY_TRIGGER_OID__
  ...
  if v_function_hash <> 'bbe1c2892c5191e820a36c64933585e5172f5289945540efae79ed0ae5547ae6'
     or v_trigger_hash <> 'e89ac15de9a1f20a2bd269289fa3ddf3ef22ff529d8c4b929587da5781995bbf'
     or v_function_hash <> '__ENTRY_FUNCTION_SHA256__' or v_trigger_hash <> '__ENTRY_TRIGGER_SHA256__' then
    raise exception 'Phase 03B exit refused: gate definition changed.' using errcode='P0001';
  end if;

  if (select count(*) from public.flags) <> __ENTRY_FLAGS_COUNT__
     or encode(extensions.digest(coalesce((select string_agg(id::text||E'\t'||status::text,E'\n' order by id) from public.flags),''),'sha256'),'hex') <> '__ENTRY_FLAGS_SHA256__'
     or (select count(*) from public.flag_status_history) <> __ENTRY_HISTORY_COUNT__
     or encode(extensions.digest(coalesce((select string_agg(id::text||E'\t'||flag_id::text||E'\t'||coalesce(from_status,'')||E'\t'||to_status||E'\t'||to_char(created_at at time zone 'UTC','YYYY-MM-DD"T"HH24:MI:SS.US"Z"'),E'\n' order by created_at,id) from public.flag_status_history),''),'sha256'),'hex') <> '__ENTRY_HISTORY_SHA256__' then
  ...
  select encode(extensions.digest(coalesce(string_agg(coalesce(id::text,'')||E'\t'||to_char(created at time zone 'UTC','YYYY-MM-DD"T"HH24:MI:SS.US"Z"')||E'\t'||coalesce(status_code::text,'')||E'\t'||coalesce(timed_out::text,'')||E'\t'||case when error_msg is null then 'NO_ERROR' else 'ERROR' end,E'\n' order by id nulls first,created),''),'sha256'),'hex') into v_http_hash from net._http_response;
  if (select count(*) from net.http_request_queue)<>0 or (select count(*) from net._http_response)<>6 or v_http_hash<>'709e04c5b05c3fb7986689366b007591ba9ca0740256358bbfe864314c59a2e8' then
    raise exception 'Phase 03B exit refused: HTTP baseline mismatch.' using errcode='P0001';
  end if;
end
$guard$;

select jsonb_build_object('receipt','phase03b_quiescence_pre_exit_r2','captured_at_utc',to_char(pg_catalog.clock_timestamp() at time zone 'UTC','YYYY-MM-DD"T"HH24:MI:SS.US"Z"'),'function_oid',__ENTRY_FUNCTION_OID__,'trigger_oid',__ENTRY_TRIGGER_OID__,'flags_count',__ENTRY_FLAGS_COUNT__,'flags_sha256','__ENTRY_FLAGS_SHA256__','history_count',__ENTRY_HISTORY_COUNT__,'history_sha256','__ENTRY_HISTORY_SHA256__') as phase03b_quiescence_pre_exit_r2;
...
begin
  if to_regprocedure('private.flagstone_phase03b_block_row_lifecycle_r2()') is not null
     or exists(select 1 from pg_catalog.pg_trigger where tgrelid='public.flags'::pg_catalog.regclass and tgname='aaa_flagstone_phase03b_row_lifecycle_quiescence_r2' and not tgisinternal) then
