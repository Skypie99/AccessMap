...
const r = envelope.phase03b_quiescence_entry_r2 ?? envelope;
const exact = {
...
for (const key of ['function_oid','trigger_oid','flags_id_status_count','history_count']) if (!Number.isSafeInteger(Number(r[key])) || Number(r[key]) < 0) throw new Error(`Invalid numeric entry field: ${key}`);
