...
function expectBlocked(sql, label) {
  try {
    ...
    if (!stderr.includes('Flag status changes are temporarily paused for maintenance.')) {
...
try {
  ...
  const stateSql = `select json_build_object(
    ...
    'trigger', (select tgenabled from pg_trigger where tgrelid='public.flags'::regclass and tgname='aaa_phase03b_status_write_quiescence' and not tgisinternal)
  )`;
  ...
  result.checks.statusAndHistoryStableWhileQuiesced =
    held.status === before.status && held.history === before.history && held.trigger === 'A';

  apply(path.join(PACKET, 'PROPOSED_QUIESCENCE_EXIT.sql'));
