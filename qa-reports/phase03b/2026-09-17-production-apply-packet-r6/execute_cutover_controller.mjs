...
// arming argument below. This R6 repair does not execute it against production.
...
const ARMING_VALUE = 'FLAGSTONE-P03B-R6-PRODUCTION-APPLY';
...
const RESTORATION_DISPATCH_TOKEN = Symbol('phase03b-r6-restoration-dispatch');
...
async function monitorOnce(slot, deadlines) {
  ...
  const sample = resultRow(parseCliJson(readFileSync(step.stdoutPath, 'utf8')), 'phase03b_monitor_r6');
...
async function adjudicate(reason) {
  ...
  if (result) validateR6Envelope(result, { phase: 'SERVER_STATE_CLASSIFICATION', runId, controllerPid: process.pid, controllerMonotonicOrigin: receipt.monotonicOriginMs });
...
try {
  ...
  validateR6Envelope(postApplyReceipt, {
  ...
  if (postApplyReceipt.status !== 'PASS_WHILE_QUIESCED' || postApplyReceipt.numericExit !== 0) {
    throw new Error('Post-apply comparator did not return strict R6 PASS while quiesced');
  }
  if (monitorFailure) throw monitorFailure;
