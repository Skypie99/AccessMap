...
async function run(label, script) {
  ...
  const child = spawn(process.execPath, [join(PACKET, script)], { stdio: ['ignore', out, err], env: { ...process.env } });
...
try {
  ...
  receipt.children.push(await run('LOCAL_R6_CONTROL_VALIDATION', 'validate_r6_controls.mjs'));
  const checks = receipt.children.flatMap((child) => [
  ...
  receipt.localReplayCheckCount = checks.length;
  const branchChild = receipt.children.find((child) => child.label === 'LOCAL_R6_CONTROL_VALIDATION');
  ...
  receipt.newR6BranchTests = {
    passed: branchChild?.payload?.newR6BranchPassed ?? 0,
    total: branchChild?.payload?.newR6BranchTotal ?? 0,
  };
...
console.log(JSON.stringify({
  ...
  newR6BranchTests: receipt.newR6BranchTests,
