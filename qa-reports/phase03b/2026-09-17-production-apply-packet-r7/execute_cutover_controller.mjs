...
// arming argument below. This R7 repair does not execute it against production.
...
const ARMING_VALUE = 'FLAGSTONE-P03B-R7-PRODUCTION-APPLY';
...
const RESTORATION_DISPATCH_TOKEN = Symbol('phase03b-r7-restoration-dispatch');
...
async function monitorOnce(slot, deadlines) {
  ...
  const sample = resultRow(parseCliJson(readFileSync(step.stdoutPath, 'utf8')), 'phase03b_monitor_r7');
...
try {
  ...
  validateR7Envelope(postApplyReceipt, {
  ...
  if (postApplyReceipt.status !== 'PASS_WHILE_QUIESCED' || postApplyReceipt.numericExit !== 0) {
    throw new Error('Post-apply comparator did not return strict R7 PASS while quiesced');
  }
  if (monitorFailure) throw monitorFailure;
