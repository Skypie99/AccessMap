#!/usr/bin/env node
// Disposable, local-only executable validation of the two R7 repairs while
...
} from './r7_control_lib.mjs';
import { validateCompiledR7Schema } from './r7_schema_validator.mjs';

const PACKET = dirname(fileURLToPath(import.meta.url));
const temp = mkdtempSync(join(tmpdir(), 'p03b-r7-controls-'));
const result = {
  status: 'ERROR',
  localOnly: true,
...
function exactGateSnapshot() {
  ...
  return {
    querySucceeded: true,
    receipt: 'phase03b_server_state_r7',
...
try {
  ...
  const variants = {
    case01ExactR7EnvelopeAccepted: () => validateEntryEnvelope(envelope),
    case02BareScalarRejected: () => expectFailure(() => validateR7Envelope(7), 'bare scalar'),
    case03BareHashRejected: () => expectFailure(() => validateR7Envelope('a'.repeat(64)), 'bare hash'),
    case04EmptyObjectRejected: () => expectFailure(() => validateR7Envelope({}), 'empty object'),
    case05R4EnvelopeRejected: () => { const value = clone(envelope); value.packetVersion = 'R4'; return expectFailure(() => validateR7Envelope(value), 'R4 envelope'); },
    case06UnversionedEnvelopeRejected: () => { const value = clone(envelope); delete value.packetVersion; return expectFailure(() => validateR7Envelope(value), 'unversioned envelope'); },
    case07MissingCandidateShaRejected: () => { const value = clone(envelope); delete value.candidateSha; return expectFailure(() => validateR7Envelope(value), 'missing candidateSha'); },
    case08WrongCandidateShaRejected: () => { const value = clone(envelope); value.candidateSha = '0'.repeat(40); return expectFailure(() => validateR7Envelope(value), 'wrong candidateSha'); },
    case09WrongCandidateTreeRejected: () => { const value = clone(envelope); value.candidateTree = '0'.repeat(40); return expectFailure(() => validateR7Envelope(value), 'wrong candidateTree'); },
    case10WrongProductionTargetRejected: () => { const value = clone(envelope); value.productionTarget = EXPECTED.forbiddenStagingTarget; return expectFailure(() => validateR7Envelope(value), 'wrong production target'); },
    case11MissingRunIdRejected: () => { const value = clone(envelope); delete value.runId; return expectFailure(() => validateR7Envelope(value), 'missing runId'); },
    case12MissingMonotonicTimestampRejected: () => { const value = clone(envelope); delete value.capturedAtMonotonic; return expectFailure(() => validateR7Envelope(value), 'missing monotonic timestamp'); },
    case13MissingNumericExitRejected: () => { const value = clone(envelope); delete value.numericExit; return expectFailure(() => validateR7Envelope(value), 'missing numeric exit'); },
    case14MalformedStatusRejected: () => { const value = clone(envelope); value.status = 'MAYBE'; return expectFailure(() => validateR7Envelope(value), 'malformed status'); },
    case15UnknownSchemaVersionRejected: () => { const value = clone(envelope); value.schemaVersion = 'legacy'; return expectFailure(() => validateR7Envelope(value), 'unknown schema'); },
    case16UnexpectedLegacyAliasRejected: () => { const value = clone(envelope); value.phase03b_quiescence_entry_r3 = value.observed.entry; return expectFailure(() => validateR7Envelope(value), 'legacy alias'); },
  };
  ...
  const rejectPostApply = (value, label) => expectFailure(() => validateR7Envelope(value, {
    phase: 'POST_APPLY_COMPARATOR', ...comparatorConstraints,
  }), label);
  result.checks.strictEnvelopeUsedPostApply = validateR7Envelope(postApplyEnvelope, { phase: 'POST_APPLY_COMPARATOR', ...comparatorConstraints }) === 'VALIDATED_R7';
  result.checks.strictEnvelopeUsedPostExit = validateR7Envelope(postExitEnvelope, { phase: 'POST_EXIT_COMPARATOR', ...comparatorConstraints }) === 'VALIDATED_R7';
  ...
  d1.d1_01_exactR7EnvelopeAccepted = validateR7Envelope(postApplyEnvelope, { phase: 'POST_APPLY_COMPARATOR', ...comparatorConstraints }) === 'VALIDATED_R7';
  ...
  d1.d1_04_bareScalarRejected = expectFailure(() => validateR7Envelope(0), 'bare scalar');
  d1.d1_05_bareHashRejected = expectFailure(() => validateR7Envelope('0'.repeat(64)), 'bare hash');
  ...
  const r7 = result.r7ValidatorCases;
  r7.r7_01_compiledSchemaAcceptsExactEntry = validateCompiledR7Schema(envelope) === 'VALIDATED_R7';
  r7.r7_02_compiledSchemaRejectsExtraTopLevel = (() => { const value = clone(envelope); value.extra = true; return expectFailure(() => validateCompiledR7Schema(value), 'compiled extra top-level'); })();
  r7.r7_03_compiledSchemaRejectsExtraNested = (() => { const value = clone(envelope); value.observed.entry.extra = true; return expectFailure(() => validateCompiledR7Schema(value), 'compiled extra nested'); })();
  r7.r7_04_compiledSchemaRejectsMissingRequired = (() => { const value = clone(envelope); delete value.observed.immediatePostCommitProof.table_oid; return expectFailure(() => validateCompiledR7Schema(value), 'compiled missing nested'); })();
  r7.r7_05_legacyAliasRejected = (() => { const value = clone(envelope); value.observed.entry.entryCommitted = true; return rejectEntry(value, 'legacy alias'); })();
  r7.r7_06_legacyR6EnvelopeRejected = (() => { const value = clone(envelope); value.packetVersion = 'R6'; return rejectEntry(value, 'R6 envelope'); })();
  r7.r7_07_wrongSchemaVersionRejected = (() => { const value = clone(envelope); value.schemaVersion = 'flagstone.phase03b.r6-envelope.v1'; return rejectEntry(value, 'wrong schema'); })();
  r7.r7_08_validExactEntryAccepted = validateEntryEnvelope(envelope, serverContext) === 'VALIDATED_R7';
  r7.r7_09_entryPassNonzeroExitRejected = (() => { const value = clone(envelope); value.numericExit = 1; return rejectEntry(value, 'entry nonzero'); })();
  r7.r7_10_entryPassTimedOutRejected = (() => { const value = clone(envelope); value.timedOut = true; return rejectEntry(value, 'entry timeout'); })();
  r7.r7_11_entryPassSignalRejected = (() => { const value = clone(envelope); value.signal = 'SIGTERM'; return rejectEntry(value, 'entry signal'); })();
  r7.r7_12_entryMissingSuccessfulStepRejected = (() => { const value = clone(envelope); delete value.observed.rawCapture.proofStep; return rejectEntry(value, 'entry missing step'); })();
  r7.r7_13_entryNestedUnknownKeyRejected = (() => { const value = clone(envelope); value.observed.entry.unexpected_nested_key = 'accepted'; return rejectEntry(value, 'review probe nested key'); })();
  r7.r7_14_entryContradictoryNestedTimeoutRejected = (() => { const value = clone(envelope); value.observed.rawCapture.entryStep.timedOut = true; value.observed.rawCapture.entryStep.signalSent = 'SIGTERM'; return rejectEntry(value, 'nested step contradiction'); })();
  r7.r7_15_entryProofIdentityMismatchRejected = (() => { const value = clone(envelope); value.observed.immediatePostCommitProof.function_oid += 1; return rejectEntry(value, 'entry proof mismatch'); })();
  r7.r7_16_crossRunEntryRejected = (() => { const value = clone(envelope); return expectFailure(() => validateEntryEnvelope(value, { ...serverContext, runId: '00000000-0000-4000-8000-000000000006' }), 'cross-run entry'); })();
  r7.r7_17_wrongCandidateEntryRejected = (() => { const value = clone(envelope); value.candidateSha = '0'.repeat(40); return rejectEntry(value, 'wrong candidate entry'); })();
  r7.r7_18_wrongTargetEntryRejected = (() => { const value = clone(envelope); value.productionTarget = EXPECTED.forbiddenStagingTarget; return rejectEntry(value, 'wrong target entry'); })();
  r7.r7_19_rawInvalidEntryCannotMaterialize = (() => { const value = clone(envelope); value.numericExit = 9; return expectFailure(() => entryFromEnvelope(value), 'raw entry bypass'); })();
  r7.r7_20_validExactServerAccepted = validateServerStateEnvelope(exactServerEnvelope, serverContext) === 'VALIDATED_R7';
  r7.r7_21_serverExtraKeyRejected = (() => { const value = clone(exactServerEnvelope); value.observed.gateState.extra = true; return rejectServer(value, 'server extra key'); })();
  r7.r7_22_serverMissingIdentityFieldRejected = (() => { const value = clone(exactServerEnvelope); delete value.observed.snapshot.function_definition_sha256; return rejectServer(value, 'server missing identity'); })();
  r7.r7_23_serverClassificationContradictionRejected = (() => { const value = clone(exactServerEnvelope); value.observed.entryClassification = 'ENTRY_CONFIRMED_NOT_COMMITTED'; value.status = 'ENTRY_CONFIRMED_NOT_COMMITTED'; return rejectServer(value, 'server classification contradiction'); })();
  r7.r7_24_gateHashMismatchCannotClaimCommitted = (() => { const value = clone(exactServerEnvelope); value.observed.snapshot.function_definition_sha256 = '0'.repeat(64); return rejectServer(value, 'hash mismatch committed'); })();
  r7.r7_25_absentClassificationWithLiveObjectsRejected = (() => { const value = clone(absentServerEnvelope); value.observed.snapshot.function_oid = envelope.observed.entry.function_oid; return rejectServer(value, 'absent with live object'); })();
  r7.r7_26_committedClassificationIncompleteObjectsRejected = (() => { const value = clone(exactServerEnvelope); delete value.observed.snapshot.row_trigger_oid; return rejectServer(value, 'committed incomplete objects'); })();
  r7.r7_27_unknownEvidenceCannotBeDefinitive = (() => { const value = clone(unknownServerEnvelope); value.observed.entryClassification = 'ENTRY_CONFIRMED_NOT_COMMITTED'; value.status = 'ENTRY_CONFIRMED_NOT_COMMITTED'; return rejectServer(value, 'unknown definitive'); })();
  r7.r7_28_crossRunServerRejected = expectFailure(() => validateServerStateEnvelope(exactServerEnvelope, { ...serverContext, runId: '00000000-0000-4000-8000-000000000006' }), 'cross-run server');
  r7.r7_29_wrongCandidateServerRejected = (() => { const value = clone(exactServerEnvelope); value.candidateSha = '0'.repeat(40); return rejectServer(value, 'wrong candidate server'); })();
  r7.r7_30_wrongTargetServerRejected = (() => { const value = clone(exactServerEnvelope); value.productionTarget = EXPECTED.forbiddenStagingTarget; return rejectServer(value, 'wrong target server'); })();
  r7.r7_31_rawInvalidServerCannotDriveAdjudication = (() => { const value = clone(exactServerEnvelope); value.observed.snapshot.querySucceeded = false; return rejectServer(value, 'raw server bypass'); })();
  r7.r7_32_validGateAbsentClassificationAccepted = validateServerStateEnvelope(absentServerEnvelope, serverContext) === 'VALIDATED_R7';
  r7.r7_33_validUnknownFailClosedClassificationAccepted = validateServerStateEnvelope(unknownServerEnvelope, serverContext) === 'VALIDATED_R7';
  r7.r7_34_serverPolicyMismatchRejected = (() => { const value = clone(exactServerEnvelope); value.observed.policy = statePolicy('BACKEND_UNKNOWN'); return rejectServer(value, 'server policy mismatch'); })();
  r7.r7_35_serverApplySpawnedContextMismatchRejected = expectFailure(() => validateServerStateEnvelope(exactServerEnvelope, { ...serverContext, applySpawned: true }), 'apply context mismatch');
  ...
  const r7ValidatorPass = Object.values(result.r7ValidatorCases).every(Boolean) && Object.keys(result.r7ValidatorCases).length >= 30;
  ...
  result.r7ValidatorPassed = Object.values(result.r7ValidatorCases).filter(Boolean).length;
  result.r7ValidatorTotal = Object.keys(result.r7ValidatorCases).length;
  result.status = requiredPass && preservedR6Pass && r7ValidatorPass && Object.values(result.checks).every(Boolean) ? 'PASS' : 'HOLD';
} catch (error) {
