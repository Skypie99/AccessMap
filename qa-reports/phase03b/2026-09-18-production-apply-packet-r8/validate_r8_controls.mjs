#!/usr/bin/env node
// Disposable, local-only executable validation of the two R8 repairs while
...
const temp = mkdtempSync(join(tmpdir(), 'p03b-r8-controls-'));
const result = {
  status: 'ERROR',
  localOnly: true,
...
try {
  ...
  r8Schema.r8s_10_currentRowsWrapperExtracted = resultRow({ rows: [{ phase03b_server_state_r8: liveSnapshot }] }, 'phase03b_server_state_r8') === liveSnapshot &&
    expectFailure(() => resultRow({ rows: [{ phase03b_server_state_r8: liveSnapshot }, { phase03b_server_state_r8: liveSnapshot }] }, 'phase03b_server_state_r8'), 'multiple CLI rows');
  ...
  result.status = requiredPass && preservedR6Pass && r8ValidatorPass && r8SchemaRepairPass && Object.values(result.checks).every(Boolean) ? 'PASS' : 'HOLD';
} catch (error) {
