-- PROPOSAL ONLY. Privacy-safe R8 monitor; read-only and target-explicit at invocation.
-- The controller schedules this at monotonic T+5, T+10, ... and enforces every result.
...
select jsonb_build_object(
  ...
  'trigger_count',(select count(*) from row_t),'gate_trigger_oid',(select oid from row_t),'gate_table_owner',(select table_owner from row_t),'gate_enabled',(select tgenabled from row_t),
  'gate_definition_sha256',(select encode(extensions.digest(jsonb_build_object('schema',nspname,'table',relname,'table_owner',table_owner,'name',tgname,'enabled',tgenabled,'type',tgtype,'condition',condition,'definition',definition,'function_schema',function_schema,'function_name',function_name,'function_args',function_args)::text,'sha256'),'hex') from row_t),
  'truncate_trigger_count',(select count(*) from truncate_t),'truncate_gate_trigger_oid',(select oid from truncate_t),'truncate_gate_enabled',(select tgenabled from truncate_t),
  'truncate_gate_definition_sha256',(select encode(extensions.digest(jsonb_build_object('schema',nspname,'table',relname,'table_owner',table_owner,'name',tgname,'enabled',tgenabled,'type',tgtype,'condition',condition,'definition',definition,'function_schema',function_schema,'function_name',function_name,'function_args',function_args)::text,'sha256'),'hex') from truncate_t),
