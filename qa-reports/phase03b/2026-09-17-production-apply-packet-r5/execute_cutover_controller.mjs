...
// arming argument below. This R5 repair does not execute it against production.
...
const ARMING_VALUE = 'FLAGSTONE-P03B-R5-PRODUCTION-APPLY';
...
async function monitorOnce(slot, deadlines) {
  ...
  const sample = resultRow(parseCliJson(readFileSync(step.stdoutPath, 'utf8')), 'phase03b_monitor_r5');
...
async function adjudicate(reason) {
  ...
  if (existsSync(directory)) {
    ...
    return existsSync(existing) ? readJson(existing) : null;
  }
  const args = [
  ...
  if (result) validateR5Envelope(result, { phase: 'SERVER_STATE_CLASSIFICATION', runId, controllerPid: process.pid, controllerMonotonicOrigin: receipt.monotonicOriginMs });
...
try {
  ...
  validateR5Envelope(postApplyReceipt, {
    phase: 'POST_APPLY_COMPARATOR', runId, controllerPid: process.pid, controllerMonotonicOrigin: originMonoMs,
  });
  if (postApplyReceipt.status !== 'PASS_WHILE_QUIESCED' || postApplyReceipt.numericExit !== 0) {
    throw new Error('Post-apply comparator did not return strict R5 PASS while quiesced');
  }
  if (monitorFailure) throw monitorFailure;
  ...
  assertExitDispatchAllowed({
    ...
    allComparatorsPass: true, noUnknownState: receipt.result !== 'UNKNOWN' && !interrupted, gateIdentityExact: true,
  });
  ...
  setControllerState('EXIT_DISPATCHED_PRE_DEADLINE', 'Atomic exit may complete if the 600-second boundary is crossed after this dispatch');
  ...
  const disposition = postExitDisposition(postExitStep, postExitReceipt, {
    runId, controllerPid: process.pid, controllerMonotonicOrigin: originMonoMs,
  });
