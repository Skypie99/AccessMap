#!/usr/bin/env node
// Disposable, local-only executable validation of the four R5 repairs.
...
import {
  ...
  validateR5Envelope,
} from './r5_control_lib.mjs';

const PACKET = dirname(fileURLToPath(import.meta.url));
const temp = mkdtempSync(join(tmpdir(), 'p03b-r5-controls-'));
const result = {
  status: 'ERROR',
  localOnly: true,
...
function exactGateSnapshot() {
  ...
  return {
    querySucceeded: true,
    receipt: 'phase03b_server_state_r5',
    transaction_read_only: 'on',
    table_oid: 40999,
...
try {
  ...
  const variants = {
    case01ExactR5EnvelopeAccepted: () => validateEntryEnvelope(envelope),
    case02BareScalarRejected: () => expectFailure(() => validateR5Envelope(7), 'bare scalar'),
    case03BareHashRejected: () => expectFailure(() => validateR5Envelope('a'.repeat(64)), 'bare hash'),
    case04EmptyObjectRejected: () => expectFailure(() => validateR5Envelope({}), 'empty object'),
    case05R4EnvelopeRejected: () => { const value = clone(envelope); value.packetVersion = 'R4'; return expectFailure(() => validateR5Envelope(value), 'R4 envelope'); },
    case06UnversionedEnvelopeRejected: () => { const value = clone(envelope); delete value.packetVersion; return expectFailure(() => validateR5Envelope(value), 'unversioned envelope'); },
    case07MissingCandidateShaRejected: () => { const value = clone(envelope); delete value.candidateSha; return expectFailure(() => validateR5Envelope(value), 'missing candidateSha'); },
    case08WrongCandidateShaRejected: () => { const value = clone(envelope); value.candidateSha = '0'.repeat(40); return expectFailure(() => validateR5Envelope(value), 'wrong candidateSha'); },
    case09WrongCandidateTreeRejected: () => { const value = clone(envelope); value.candidateTree = '0'.repeat(40); return expectFailure(() => validateR5Envelope(value), 'wrong candidateTree'); },
    case10WrongProductionTargetRejected: () => { const value = clone(envelope); value.productionTarget = EXPECTED.forbiddenStagingTarget; return expectFailure(() => validateR5Envelope(value), 'wrong production target'); },
    case11MissingRunIdRejected: () => { const value = clone(envelope); delete value.runId; return expectFailure(() => validateR5Envelope(value), 'missing runId'); },
    case12MissingMonotonicTimestampRejected: () => { const value = clone(envelope); delete value.capturedAtMonotonic; return expectFailure(() => validateR5Envelope(value), 'missing monotonic timestamp'); },
    case13MissingNumericExitRejected: () => { const value = clone(envelope); delete value.numericExit; return expectFailure(() => validateR5Envelope(value), 'missing numeric exit'); },
    case14MalformedStatusRejected: () => { const value = clone(envelope); value.status = 'MAYBE'; return expectFailure(() => validateR5Envelope(value), 'malformed status'); },
    case15UnknownSchemaVersionRejected: () => { const value = clone(envelope); value.schemaVersion = 'legacy'; return expectFailure(() => validateR5Envelope(value), 'unknown schema'); },
    case16UnexpectedLegacyAliasRejected: () => { const value = clone(envelope); value.phase03b_quiescence_entry_r3 = value.observed.entry; return expectFailure(() => validateR5Envelope(value), 'legacy alias'); },
  };
  ...
  const allowedAt599999 = assertExitDispatchAllowed({
    controllerState: 'VERIFIED_SAFE_TO_EXIT', escalationLatch: beforeLatch,
    nowMonoMs: origin + 599_999, maximumMonoMs: deadlines.maximumQuiescenceEscalationMonoMs,
    allComparatorsPass: true, noUnknownState: true, gateIdentityExact: true,
  });
  ...
  result.requiredBranchCases.case29Cross600BeforeDispatchBlocked = expectFailure(() => assertExitDispatchAllowed({
    controllerState: 'VERIFIED_SAFE_TO_EXIT', escalationLatch: boundaryLatch,
  ...
  assertExitDispatchAllowed({
    controllerState: 'VERIFIED_SAFE_TO_EXIT', escalationLatch: delayedLatch,
  ...
  result.checks.strictEnvelopeUsedPostApply = validateR5Envelope(postApplyEnvelope, { phase: 'POST_APPLY_COMPARATOR', runId });
  result.checks.strictEnvelopeUsedPostExit = validateR5Envelope(postExitEnvelope, { phase: 'POST_EXIT_COMPARATOR', runId });
  result.checks.postExitStrictPassAccepted = postExitDisposition({ timedOut: false, exitCode: 0, exitSignal: null, spawnError: null }, postExitEnvelope, { runId }).result === 'PASS';
  ...
  result.checks.controllerHasImmediatePreDispatchGuard = controllerSource.indexOf("setControllerState('EXIT_DISPATCHED_PRE_DEADLINE'") > controllerSource.lastIndexOf('assertExitDispatchAllowed({');
  ...
  result.status = requiredPass && Object.values(result.checks).every(Boolean) ? 'PASS' : 'HOLD';
} catch (error) {
