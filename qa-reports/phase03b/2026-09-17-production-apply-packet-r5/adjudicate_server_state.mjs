#!/usr/bin/env node
// Read-only R5 adjudicator for timeout, lost output, signals, terminal/network
...
try {
  ...
  snapshot = resultRow(parseCliJson(readFileSync(stateStep.stdoutPath, 'utf8')), 'phase03b_server_state_r5');
  ...
  if (entryEnvelope && exactPair && !(snapshot.candidate_backends ?? []).some((backend) => backend.state !== 'idle') && gateState.state === 'GATE_PRESENT_EXACT') {
    ...
    try {
      validateR5Envelope(comparatorEnvelope, { phase: 'POST_APPLY_COMPARATOR', runId, controllerPid, controllerMonotonicOrigin });
  ...
  snapshot = { ...snapshot, querySucceeded: false, captureError: error.message };
