...
try {
  ...
  psql(['-q', '-c', `
    ...
    insert into net._http_response(id,created,status_code,timed_out,error_msg) values
      (73,'2026-09-19T07:50:52.527715Z',200,false,null),
      (74,'2026-09-19T07:51:02.898610Z',400,false,null),
      (75,'2026-09-19T07:51:13.807611Z',400,false,null),
      (76,'2026-09-19T07:51:22.334513Z',400,false,null),
      (77,'2026-09-19T07:51:27.802633Z',200,false,null),
      (78,'2026-09-19T07:53:20.534177Z',200,false,null);
  `]);

  const preflight = JSON.parse(psql([
    '-qAt', '-f', path.join(PACKET, 'PG_NET_RUN_RELATIVE_PREFLIGHT.sql'),
  ]).split('\n').find((line) => line.startsWith('{')));
  const pgNetContext = createRunRelativePgNetContext(preflight);
  const boundSql = {};
  for (const [key, filename] of [
    ['entry', 'PROPOSED_QUIESCENCE_ENTER.sql'],
    ['proof', 'PROPOSED_QUIESCENCE_VERIFY.sql'],
    ['serverState', 'SERVER_STATE_ADJUDICATE.sql'],
    ['postExit', 'POST_EXIT_VERIFY.sql'],
  ]) {
    const output = path.join(tmp, `bound-${filename}`);
    fs.writeFileSync(
      output,
      bindDatabaseT0Sql(fs.readFileSync(path.join(PACKET, filename), 'utf8'), pgNetContext.databaseT0),
      { mode: 0o600, flag: 'wx' },
    );
    boundSql[key] = output;
  }
  result.databaseT0 = pgNetContext.databaseT0;
  result.pgNetTtl = pgNetContext.pgNetTtl;
  result.checks.pgNetPreflightUsesDatabaseT0 = Object.isFrozen(pgNetContext) &&
    preflight.transaction_read_only === 'on' && preflight.http_queue_count === 0 &&
    preflight.http_response_new_since_t0_count === 0;
  ...
  const entryOutput = psql(['-qAt', '-f', boundSql.entry]);
  ...
  const immediateProof = JSON.parse(psql(['-qAt', '-f', boundSql.proof]).split('\n').find((line) => line.startsWith('{')));
  ...
  const entryEnvelope = buildEntryEnvelope({
    ...
    entryStep: {
      label: '01-entry/entry', command: ['supabase', 'db', '--project-ref', 'kldlwszpfkdmsjrjhjym'],
      controllerPid: process.pid, childPid: process.pid, processGroupId: null,
      startedAt: new Date().toISOString(), endedAt: new Date().toISOString(),
      startedMonoMs: originMonoMs, endedMonoMs: originMonoMs, absoluteDeadlineMonoMs: originMonoMs + 30_000,
      restorationDispatchedAtMonoMs: null, timeoutMs: 30_000, timedOut: false, signalSent: null,
      exitCode: 0, exitSignal: null, spawnError: null, stdoutPath: '/local/entry.stdout', stderrPath: '/local/entry.stderr',
    },
    proofStep: {
      label: '01-entry/immediate-proof', command: ['supabase', 'db', '--project-ref', 'kldlwszpfkdmsjrjhjym'],
      controllerPid: process.pid, childPid: process.pid, processGroupId: null,
      startedAt: new Date().toISOString(), endedAt: new Date().toISOString(),
      startedMonoMs: originMonoMs, endedMonoMs: originMonoMs, absoluteDeadlineMonoMs: originMonoMs + 30_000,
      restorationDispatchedAtMonoMs: null, timeoutMs: 30_000, timedOut: false, signalSent: null,
      exitCode: 0, exitSignal: null, spawnError: null, stdoutPath: '/local/proof.stdout', stderrPath: '/local/proof.stderr',
    },
    capturedAtUtc: new Date().toISOString(),
    capturedAtMonotonic: originMonoMs,
    pgNetContext,
  });
  result.checks.controllerEnvelopeGeneratedFromRawEntryAndProof = entryEnvelope.packetVersion === 'R8';
  const adjudicationSnapshot = JSON.parse(psql(['-qAt', '-f', boundSql.serverState]).split('\n').find((line) => line.startsWith('{')));
  adjudicationSnapshot.querySucceeded = true;
  const adjudicatedGate = classifyGateIdentity(adjudicationSnapshot, entry);
  result.adjudicatedGate = adjudicatedGate;
  result.checks.serverAdjudicationPinsExactGate = adjudicatedGate.state === 'GATE_PRESENT_EXACT';
  result.checks.serverAdjudicationCapturesDefinitionHashes =
    adjudicationSnapshot.function_definition_sha256 === entry.function_definition_sha256 &&
    adjudicationSnapshot.row_trigger_definition_sha256 === entry.trigger_definition_sha256 &&
    adjudicationSnapshot.truncate_trigger_definition_sha256 === entry.truncate_trigger_definition_sha256;
  ...
  const proof = JSON.parse(psql(['-qAt', '-f', boundSql.proof]).split('\n').find((line) => line.startsWith('{')));
