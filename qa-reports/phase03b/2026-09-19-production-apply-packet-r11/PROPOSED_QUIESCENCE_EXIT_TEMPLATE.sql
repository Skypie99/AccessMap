...
begin
  ...
  if 'private.flagstone_phase03b_block_row_lifecycle_r2()'::pg_catalog.regprocedure::oid <> __ENTRY_FUNCTION_OID__
     ...
     or (select oid from pg_catalog.pg_trigger where tgrelid='public.flags'::pg_catalog.regclass and tgname='aaa_flagstone_phase03b_truncate_quiescence_r3' and not tgisinternal) <> __ENTRY_TRUNCATE_TRIGGER_OID__
  ...
  if v_function_hash <> '16555e58ee2cdfce5d54336ef63584116ed1cb798d90ac779c278a5c0e5b0bac'
     ...
     or v_function_hash <> '__ENTRY_FUNCTION_SHA256__' or v_trigger_hash <> '__ENTRY_TRIGGER_SHA256__'
     or v_truncate_trigger_hash <> '__ENTRY_TRUNCATE_TRIGGER_SHA256__' then
...
select jsonb_build_object('receipt','phase03b_quiescence_pre_exit_r3','captured_at_utc',to_char(pg_catalog.clock_timestamp() at time zone 'UTC','YYYY-MM-DD"T"HH24:MI:SS.US"Z"'),'database_t0',to_char('__DATABASE_T0__'::timestamptz at time zone 'UTC','YYYY-MM-DD"T"HH24:MI:SS.US"Z"'),'pg_net_ttl',current_setting('pg_net.ttl', true),'pg_net_ttl_seconds',extract(epoch from current_setting('pg_net.ttl')::interval)::bigint,'http_queue_count',(select count(*) from net.http_request_queue),'http_response_count',(select count(*) from net._http_response),'http_response_sha256',(select encode(extensions.digest(coalesce(string_agg(coalesce(id::text,'')||E'\t'||to_char(created at time zone 'UTC','YYYY-MM-DD"T"HH24:MI:SS.US"Z"')||E'\t'||coalesce(status_code::text,'')||E'\t'||coalesce(timed_out::text,'')||E'\t'||case when error_msg is null then 'NO_ERROR' else 'ERROR' end,E'\n' order by id nulls first,created),''),'sha256'),'hex') from net._http_response),'http_response_new_since_t0_count',(select count(*) from net._http_response where created >= '__DATABASE_T0__'::timestamptz),'function_oid',__ENTRY_FUNCTION_OID__,'trigger_oid',__ENTRY_TRIGGER_OID__,'truncate_trigger_oid',__ENTRY_TRUNCATE_TRIGGER_OID__,'flags_count',__ENTRY_FLAGS_COUNT__,'flags_sha256','__ENTRY_FLAGS_SHA256__','history_count',__ENTRY_HISTORY_COUNT__,'history_sha256','__ENTRY_HISTORY_SHA256__') as phase03b_quiescence_pre_exit_r3;
