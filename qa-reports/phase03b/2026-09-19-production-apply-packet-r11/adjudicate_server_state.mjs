#!/usr/bin/env node
// Read-only R10 adjudicator for timeout, lost output, signals, terminal/network
...
const preflightArg = process.argv.find((value) => value.startsWith('--preflight='));
...
if (!evidenceArg || !preflightArg) throw new Error('Required: --evidence=... --preflight=... plus --entry=... or exact run context');
const pgNetPreflight = JSON.parse(readFileSync(resolve(preflightArg.slice('--preflight='.length)), 'utf8'));
const pgNetContext = createRunRelativePgNetContext(pgNetPreflight);
...
if (entryArg) {
  ...
  if (entryEnvelope.expected.databaseT0 !== pgNetContext.databaseT0) throw new Error('Entry and pg_net preflight database T0 differ');
} else {
...
try {
  const boundStateSql = join(evidence, 'BOUND_SERVER_STATE_ADJUDICATE.sql');
  writeFileSync(boundStateSql, bindDatabaseT0Sql(readFileSync(join(PACKET, 'SERVER_STATE_ADJUDICATE.sql'), 'utf8'), pgNetContext.databaseT0), { mode: 0o600, flag: 'wx' });
  const stateStep = run('server-state', 'supabase', [
    'db', 'query', '--linked', '--project-ref', EXPECTED.productionTarget,
    '--file', boundStateSql, '--output-format', 'json',
  ]);
  ...
  const extractedSnapshot = resultRow(parseCliJson(readFileSync(stateStep.stdoutPath, 'utf8')), 'phase03b_server_state_r8');
  if (!extractedSnapshot || typeof extractedSnapshot !== 'object' || Array.isArray(extractedSnapshot)) {
    throw new Error('Server-state row must contain one snapshot object');
  }
  snapshot = { ...extractedSnapshot, querySucceeded: true };
  const adjudicated = classifyValidatedServerState(snapshot, { entryEnvelope, entry, applySpawned, databaseT0: pgNetContext.databaseT0 }, {
    comparePostApply: () => {
      ...
      try {
        ...
        return comparatorStep.exitCode === 0 && !comparatorStep.signal && !comparatorStep.timedOut && comparatorEnvelope.status === 'PASS_WHILE_QUIESCED';
      } catch {
        return false;
      }
    },
  });
  gateState = adjudicated.gateState;
  entryClassification = adjudicated.entryClassification;
  applyClassification = adjudicated.applyClassification;
  policy = adjudicated.policy;
  numericExit = 0;
} catch (error) {
  snapshot = { querySucceeded: false, captureError: error.message };
  policy = statePolicy('BACKEND_UNKNOWN');
...
const envelope = buildR8Envelope({
  ...
  numericExit,
  validationContext: { runId, controllerPid, controllerMonotonicOrigin, entryEnvelope, applySpawned, databaseT0: pgNetContext.databaseT0 },
});
validateServerStateEnvelope(envelope, { runId, controllerPid, controllerMonotonicOrigin, entryEnvelope, applySpawned, databaseT0: pgNetContext.databaseT0 });
