#!/usr/bin/env node
// Disposable, local-only validation of the R10 pre-classification snapshot
// validation-order repair while preserving the accepted R9-R5 controls.
...
import {
  ...
  classifyServerState,
  classifyValidatedServerState,
  createDeadlines,
  createEscalationLatch,
  createRunRelativePgNetContext,
  deriveClientOutcome,
  ...
  postExitDisposition,
  resultRow,
  responseRemainsObservableThroughFinal,
  statePolicy,
  ...
  validateR8Envelope,
  validatePgNetRunRelativePreflight,
  validateRunRelativePgNetCheckpoint,
  validateServerStateSnapshot,
...
const temp = mkdtempSync(join(tmpdir(), 'p03b-r11-controls-'));
const DATABASE_T0 = '2026-09-19T18:00:00.000000Z';
const pgNetPreflightFixture = {
  receipt: 'phase03b_pgnet_run_relative_preflight_r11',
  captured_at_utc: DATABASE_T0,
  database_t0: DATABASE_T0,
  transaction_read_only: 'on',
  pg_net_ttl: '6 hours',
  pg_net_ttl_seconds: 21600,
  http_queue_count: 0,
  http_response_count: 6,
  http_response_sha256: 'd'.repeat(64),
  http_response_new_since_t0_count: 0,
};
const pgNetContext = createRunRelativePgNetContext(pgNetPreflightFixture);
const result = {
  status: 'ERROR',
  localOnly: true,
  ...
  r10ValidationOrderCases: {},
  r11PgNetCases: {},
  r10ValidationOrderMetrics: {
    invalidSnapshotClassifierCallCount: 0,
    invalidSnapshotComparatorCallCount: 0,
    invalidSnapshotConsumedResultCount: 0,
    invalidSnapshotControllerTransitionCount: 0,
  },
  error: null,
  tempDestroyed: false,
};
...
function comparatorEnvelope(phase, status, entryEnvelope) {
  ...
  const sharedProof = {
    ...
    http_response_sha256: 'e'.repeat(64),
    http_response_new_since_t0_count: 0,
  };
...
try {
  ...
  d1.d1_23_wrongFinalDigestsRejected = ['finalLedgerSha256', 'finalStructureSha256'].every((key) => {
  ...
  const r10 = result.r10ValidationOrderCases;
  const exerciseInvalidSnapshot = (candidate, label) => {
    const counters = { classifier: 0, comparator: 0, consumed: 0, transition: 0 };
    let rejected = false;
    try {
      const classified = classifyValidatedServerState(candidate, { entryEnvelope: {}, entry: null, applySpawned: false }, {
        classifyEntry: (...args) => { counters.classifier += 1; return classifyEntryState(...args); },
        classifyApply: (...args) => { counters.classifier += 1; return classifyServerState(...args); },
        comparePostApply: () => { counters.comparator += 1; return true; },
      });
      counters.consumed += classified ? 1 : 0;
      counters.transition += 1;
    } catch (error) {
      rejected = String(error.message).startsWith('FAIL_CLOSED_INVALID_ENVELOPE:');
    }
    result.r10ValidationOrderMetrics.invalidSnapshotClassifierCallCount += counters.classifier;
    result.r10ValidationOrderMetrics.invalidSnapshotComparatorCallCount += counters.comparator;
    result.r10ValidationOrderMetrics.invalidSnapshotConsumedResultCount += counters.consumed;
    result.r10ValidationOrderMetrics.invalidSnapshotControllerTransitionCount += counters.transition;
    return rejected && Object.values(counters).every((count) => count === 0);
  };

  r10.r10_01_validExactSnapshotValidatedThenClassified = (() => {
    let validated = false;
    let classifierCalls = 0;
    const classified = classifyValidatedServerState(clone(liveSnapshot), { entryEnvelope: null, entry: null, applySpawned: false }, {
      validateSnapshot: (candidate) => {
        const marker = validateServerStateSnapshot(candidate);
        validated = marker === 'VALIDATED_R10';
        return marker;
      },
      classifyEntry: (...args) => {
        if (!validated) throw new Error('entry classifier called before validation');
        classifierCalls += 1;
        return classifyEntryState(...args);
      },
      classifyApply: (...args) => {
        if (!validated) throw new Error('apply classifier called before validation');
        classifierCalls += 1;
        return classifyServerState(...args);
      },
    });
    return validated && classifierCalls === 2 && classified.validationMarker === 'VALIDATED_R10' &&
      classified.entryClassification === 'ENTRY_CONFIRMED_NOT_COMMITTED';
  })();
  r10.r10_02_extraTopLevelKeyZeroCalls = exerciseInvalidSnapshot({
    ...clone(liveSnapshot), unexpected_snapshot_key: true,
  }, 'extra top-level key');
  r10.r10_03_nestedUnknownKeyZeroCalls = (() => {
    const candidate = clone(liveSnapshot);
    candidate.candidate_backends = [{
      pid: 1,
      usename: 'postgres',
      application_name: '',
      state: 'idle',
    ...
    return exerciseInvalidSnapshot(candidate, 'nested unknown key');
  })();
  r10.r10_04_contradictoryDiscriminatorZeroCalls = exerciseInvalidSnapshot({
    ...clone(liveSnapshot), querySucceeded: false, captureError: 'contradictory state',
  }, 'contradictory discriminator');
  r10.r10_05_missingRequiredKeyZeroCalls = (() => {
    const candidate = clone(liveSnapshot);
    delete candidate.transaction_read_only;
    return exerciseInvalidSnapshot(candidate, 'missing required key');
  })();
  r10.r10_06_wrongSemanticIdentityZeroCalls = exerciseInvalidSnapshot({
    ...clone(liveSnapshot), receipt: 'phase03b_server_state_legacy',
  }, 'wrong semantic identity');
  r10.r10_07_malformedNestedProofZeroCalls = (() => {
    const candidate = clone(liveSnapshot);
    candidate.candidate_backends = [{
      pid: 'not-an-integer',
      usename: 'postgres',
    ...
    return exerciseInvalidSnapshot(candidate, 'malformed nested proof');
  })();
  r10.r10_08_validTransportInvalidSnapshotZeroCalls = (() => {
    const invalid = { ...clone(liveSnapshot), unexpected_snapshot_key: true };
    const extracted = resultRow([{ phase03b_server_state_r8: invalid }], 'phase03b_server_state_r8');
    return exerciseInvalidSnapshot(extracted, 'valid transport with invalid snapshot');
  })();
  r10.r10_09_validatedMarkerPrecedesAllConsumers = (() => {
    const entryFixture = entryFixtures().entry;
    const candidate = clone(exactSnapshot);
    candidate.phase03b_versions = EXPECTED.migrations.map((migration) => migration.version);
    candidate.ledger_count = EXPECTED.final.ledgerCount;
    candidate.ledger_unique_count = EXPECTED.final.ledgerCount;
    let validated = false;
    let classifierCalls = 0;
    let comparatorCalls = 0;
    const classified = classifyValidatedServerState(candidate, { entryEnvelope: {}, entry: entryFixture, applySpawned: true }, {
      validateSnapshot: (value) => {
        const marker = validateServerStateSnapshot(value);
        validated = marker === 'VALIDATED_R10';
        return marker;
      },
      classifyEntry: (...args) => {
        if (!validated) throw new Error('entry classifier called before VALIDATED_R10');
        classifierCalls += 1;
        return classifyEntryState(...args);
      },
      classifyApply: (...args) => {
        if (!validated) throw new Error('apply classifier called before VALIDATED_R10');
        classifierCalls += 1;
        return classifyServerState(...args);
      },
      comparePostApply: () => {
        if (!validated) throw new Error('comparator called before VALIDATED_R10');
        comparatorCalls += 1;
        return true;
      },
    });
    return classified.validationMarker === 'VALIDATED_R10' && classifierCalls === 2 && comparatorCalls === 1 &&
      classified.applyClassification === 'BOTH_RECORDED_EXPECTED_STRUCTURE';
  })();
  r10.r10_10_committedEntryLostOutputStillAdjudicates = (() => {
    const entryFixture = entryFixtures().entry;
    const classified = classifyValidatedServerState(clone(exactSnapshot), {
      entryEnvelope: {}, entry: entryFixture, applySpawned: false,
    });
    return classified.validationMarker === 'VALIDATED_R10' &&
      classified.entryClassification === 'ENTRY_COMMITTED_CONFIRMED' &&
      classified.gateState.state === 'GATE_PRESENT_EXACT';
  })();
  r10.r10_11_gateAbsentIdentityContradictionZeroCalls = (() => {
    const candidate = clone(liveSnapshot);
    candidate.function_oid = 424242;
    return exerciseInvalidSnapshot(candidate, 'gate-absent identity contradiction');
  })();
  r10.r10_12_allInvalidResultsUnconsumedAndNoTransition =
    result.r10ValidationOrderMetrics.invalidSnapshotClassifierCallCount === 0 &&
    result.r10ValidationOrderMetrics.invalidSnapshotComparatorCallCount === 0 &&
    result.r10ValidationOrderMetrics.invalidSnapshotConsumedResultCount === 0 &&
    result.r10ValidationOrderMetrics.invalidSnapshotControllerTransitionCount === 0;
  const adjudicatorSource = readFileSync(join(PACKET, 'adjudicate_server_state.mjs'), 'utf8');
  const controlLibrarySource = readFileSync(join(PACKET, 'r8_control_lib.mjs'), 'utf8');
  const validatedBoundarySource = controlLibrarySource.slice(
    controlLibrarySource.indexOf('export function classifyValidatedServerState'),
    controlLibrarySource.indexOf('export function assertExactInventory'),
  );
  r10.r10_13_executableUsesValidatedBoundaryOnly =
    (adjudicatorSource.match(/classifyValidatedServerState\(/g) ?? []).length === 1 &&
    !adjudicatorSource.includes('classifyEntryState(') &&
    !adjudicatorSource.includes('classifyServerState(');
  r10.r10_14_validatorSourceOrderPrecedesEveryConsumer =
    validatedBoundarySource.indexOf('const validationMarker = validateSnapshot(snapshot)') >= 0 &&
    validatedBoundarySource.indexOf('const validationMarker = validateSnapshot(snapshot)') < validatedBoundarySource.indexOf('classifyEntry(snapshot') &&
    validatedBoundarySource.indexOf('classifyEntry(snapshot') < validatedBoundarySource.indexOf('comparePostApply()') &&
    validatedBoundarySource.indexOf('comparePostApply()') < validatedBoundarySource.indexOf('classifyApply(snapshot') &&
    validatedBoundarySource.indexOf('classifyApply(snapshot') < validatedBoundarySource.indexOf('resolvePolicy(applyClassification)');
  r10.r10_15_extractedSnapshotHasNoPrevalidationPropertyReads =
    !/extractedSnapshot\s*\./.test(adjudicatorSource);
  r10.r10_16_validatorFailureHasNoContinuationPath =
    !validatedBoundarySource.includes('catch') &&
    adjudicatorSource.includes("snapshot = { querySucceeded: false, captureError: error.message }") &&
    adjudicatorSource.includes('process.exitCode = 1');

  const r11 = result.r11PgNetCases;
  const checkpoint = {
    database_t0: DATABASE_T0,
    transaction_read_only: 'on',
    pg_net_ttl: '6 hours',
    ...
    http_queue_count: 0,
    http_response_count: 6,
    http_response_sha256: 'a'.repeat(64),
    http_response_new_since_t0_count: 0,
  };
  r11.r11_01_queueZeroNoNewRowsPasses = validateRunRelativePgNetCheckpoint(checkpoint, DATABASE_T0, { requireReadOnly: true });
  r11.r11_02_oneOldRowExpiresPasses = validateRunRelativePgNetCheckpoint({ ...checkpoint, http_response_count: 5, http_response_sha256: 'b'.repeat(64) }, DATABASE_T0, { requireReadOnly: true });
  r11.r11_03_severalOldRowsExpirePasses = validateRunRelativePgNetCheckpoint({ ...checkpoint, http_response_count: 2, http_response_sha256: 'c'.repeat(64) }, DATABASE_T0, { requireReadOnly: true });
  r11.r11_04_historicalFingerprintDriftPasses = validateRunRelativePgNetCheckpoint({ ...checkpoint, http_response_sha256: 'f'.repeat(64) }, DATABASE_T0, { requireReadOnly: true });
  r11.r11_05_responseCountDecreasePasses = validateRunRelativePgNetCheckpoint({ ...checkpoint, http_response_count: 0, http_response_sha256: '0'.repeat(64) }, DATABASE_T0, { requireReadOnly: true });
  r11.r11_06_newRowAfterT0Holds = expectFailure(() => validateRunRelativePgNetCheckpoint({ ...checkpoint, http_response_new_since_t0_count: 1 }, DATABASE_T0, { requireReadOnly: true }), 'new response after T0');
  r11.r11_07_nonzeroQueueHolds = expectFailure(() => validateRunRelativePgNetCheckpoint({ ...checkpoint, http_queue_count: 1 }, DATABASE_T0, { requireReadOnly: true }), 'nonzero queue');
  r11.r11_08_pendingRequestAtPreflightHolds = expectFailure(() => validatePgNetRunRelativePreflight({ ...pgNetPreflightFixture, http_queue_count: 1 }), 'pending request preflight');
  r11.r11_09_ttlAt600Holds = expectFailure(() => validatePgNetRunRelativePreflight({ ...pgNetPreflightFixture, pg_net_ttl: '10 minutes', pg_net_ttl_seconds: 600 }), 'TTL at maximum window');
  r11.r11_10_missingOrInvalidT0Holds = [null, 'not-a-time'].every((database_t0) => expectFailure(() => validatePgNetRunRelativePreflight({ ...pgNetPreflightFixture, database_t0 }), 'invalid database T0'));
  r11.r11_11_databaseT0IgnoresLocalClock = (() => {
    const deliberatelyWrongLocalClock = '2040-01-01T00:00:00.000Z';
    return deliberatelyWrongLocalClock !== DATABASE_T0 && createRunRelativePgNetContext(pgNetPreflightFixture).databaseT0 === DATABASE_T0;
  })();
  r11.r11_12_newRowCannotExpireInsideAcceptedWindow = responseRemainsObservableThroughFinal({
    databaseT0: DATABASE_T0,
    responseCreatedAt: '2026-09-19T18:00:01.000Z',
    finalVerificationAt: '2026-09-19T18:10:00.000Z',
    ttlSeconds: 21600,
  });
  r11.r11_13_allComparatorsUseImmutableT0 = validateRunRelativePgNetCheckpoint(checkpoint, pgNetContext.databaseT0, { requireReadOnly: true }) &&
    validateRunRelativePgNetCheckpoint({ ...checkpoint, http_response_count: 1, http_response_sha256: '9'.repeat(64) }, pgNetContext.databaseT0, { requireReadOnly: true }) &&
    expectFailure(() => validateRunRelativePgNetCheckpoint({ ...checkpoint, database_t0: '2026-09-19T18:00:01.000000Z' }, pgNetContext.databaseT0, { requireReadOnly: true }), 'mutable T0');
  const safetyControlSources = [
    'r8_control_lib.mjs', 'execute_cutover_controller.mjs', 'adjudicate_server_state.mjs',
    'verify_post_apply.mjs', 'verify_post_exit.mjs', 'PROPOSED_QUIESCENCE_ENTER.sql',
    'PROPOSED_QUIESCENCE_EXIT_TEMPLATE.sql', 'PROPOSED_QUIESCENCE_VERIFY.sql',
    'MONITOR_READ_ONLY.sql', 'POST_EXIT_VERIFY.sql', 'SERVER_STATE_ADJUDICATE.sql',
  ].map((name) => readFileSync(join(PACKET, name), 'utf8')).join('\n');
  r11.r11_14_noHistoricalFingerprintEqualityInControlFlow =
    !safetyControlSources.includes('EXPECTED.baseline.httpResponse') &&
    !safetyControlSources.includes('httpResponseSha256:') &&
    !safetyControlSources.includes("net._http_response)<>6") &&
    !safetyControlSources.includes('709e04c5b05c3fb7986689366b007591ba9ca0740256358bbfe864314c59a2e8') &&
    !safetyControlSources.includes('db09cd0f61b4405a2540be7541b690df4fa52bd7697c98c1e7e88d37a3f99031') &&
    !safetyControlSources.includes('14d0617d38d7625f9470ed931c4c7e849bba5d9472bc8e39983dd1a359118dfd');
  ...
  const r10ValidationOrderPass = Object.values(result.r10ValidationOrderCases).every(Boolean) && Object.keys(result.r10ValidationOrderCases).length === 16;
  const r11PgNetPass = Object.values(result.r11PgNetCases).every(Boolean) && Object.keys(result.r11PgNetCases).length === 14;
  ...
  result.r10ValidationOrderPassed = Object.values(result.r10ValidationOrderCases).filter(Boolean).length;
  result.r10ValidationOrderTotal = Object.keys(result.r10ValidationOrderCases).length;
  result.r11PgNetPassed = Object.values(result.r11PgNetCases).filter(Boolean).length;
  result.r11PgNetTotal = Object.keys(result.r11PgNetCases).length;
  result.status = requiredPass && preservedR6Pass && r8ValidatorPass && r8SchemaRepairPass && r9TransportExactnessPass &&
    r10ValidationOrderPass && r11PgNetPass &&
