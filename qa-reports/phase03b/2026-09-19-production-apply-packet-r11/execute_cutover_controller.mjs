...
async function monitorOnce(slot, deadlines) {
  ...
  const step = await runCaptured(`03-monitoring/sample-${String(slot).padStart(3, '0')}`, 'supabase', [
    'db', 'query', '--linked', '--project-ref', TARGET, '--file', boundSqlPaths.monitor, '--output-format', 'json',
  ], deadline);
...
async function adjudicate(reason) {
  ...
  if (existsSync(directory)) {
    ...
    validateServerStateEnvelope(result, {
      ...
      entryEnvelope, applySpawned, databaseT0: pgNetContext?.databaseT0,
    });
    return result;
  }
  const args = [
    ...
    `--evidence=${directory}`,
    `--preflight=${join(evidence, '00-preflight/PG_NET_RUN_RELATIVE_PREFLIGHT.json')}`,
  ];
...
try {
  ...
  const pgNetStep = await runCaptured('00-preflight/pgnet-run-relative', 'supabase', [
    'db', 'query', '--linked', '--project-ref', TARGET, '--file', join(PACKET, 'PG_NET_RUN_RELATIVE_PREFLIGHT.sql'), '--output-format', 'json',
  ], deadlines.entryCompleteMonoMs);
  requireSuccess(pgNetStep, 'run-relative pg_net preflight');
  const pgNetPreflight = resultRow(parseCliJson(readFileSync(pgNetStep.stdoutPath, 'utf8')), 'phase03b_pgnet_run_relative_preflight_r11');
  pgNetContext = createRunRelativePgNetContext(pgNetPreflight);
  writeJson('00-preflight/PG_NET_RUN_RELATIVE_PREFLIGHT.json', pgNetPreflight);
  for (const [key, filename, output] of [
    ['entry', 'PROPOSED_QUIESCENCE_ENTER.sql', '01-entry/BOUND_QUIESCENCE_ENTER.sql'],
    ['proof', 'PROPOSED_QUIESCENCE_VERIFY.sql', '01-entry/BOUND_QUIESCENCE_VERIFY.sql'],
    ['monitor', 'MONITOR_READ_ONLY.sql', '03-monitoring/BOUND_MONITOR_READ_ONLY.sql'],
  ]) {
    const path = join(evidence, output);
    writeFileSync(path, bindDatabaseT0Sql(readFileSync(join(PACKET, filename), 'utf8'), pgNetContext.databaseT0), { flag: 'wx', mode: 0o600 });
    boundSqlPaths[key] = path;
  }
  writeJson('01-entry/RUN_CONTEXT.json', {
    ...
    pgNetTtl: pgNetContext.pgNetTtl,
    pgNetTtlSeconds: pgNetContext.pgNetTtlSeconds,
    maximumQuiescenceWindowSeconds: pgNetContext.maximumQuiescenceWindowSeconds,
  });
  ...
  const entryStep = await runCaptured('01-entry/entry', 'supabase', [
    'db', 'query', '--linked', '--project-ref', TARGET, '--file', boundSqlPaths.entry, '--output-format', 'json',
  ...
  if (Date.parse(entry.boundary_at_utc) < Date.parse(pgNetContext.databaseT0)) throw new Error('Entry boundary predates immutable database T0');

  monitorPromise = monitorLoop(deadlines);
  const proofStep = await runCaptured('01-entry/immediate-proof', 'supabase', [
    'db', 'query', '--linked', '--project-ref', TARGET, '--file', boundSqlPaths.proof, '--output-format', 'json',
  ...
  entryEnvelope = buildEntryEnvelope({
    ...
    capturedAtUtc: iso(), capturedAtMonotonic: monoMs(), pgNetContext,
  });
