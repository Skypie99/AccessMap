...
Side facts: the BUMBAKLOT flag belongs to `[TEST_ACCOUNT_1]` (uid `[REDACTED_USER_ID]`), *not* `[OWNER_ACCOUNT]` (uid `[REDACTED_USER_ID]`) — "Reported by: You" is healthy uid-equality (`FlagDetailModal.tsx:513`), so the device was signed in as [TEST_ACCOUNT_1] and the delete failed anyway. The same missing grant also breaks: admin flag delete, owner **photo** delete + admin photo delete on Storage (both storage policies since the C-12 apply 2026-07-29 / admin_role apply), and the `useIsAdmin()` gate (it can never read the column → Admin renders for nobody, even after an is_admin=true grant).
