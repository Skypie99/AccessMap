...
- Initial state: switch **ON** (`value='1'`) — code comment confirms the state mirrors `push_tokens` row presence, so the signed-in account already had a row.
...
Turning the switch OFF deleted the real `push_tokens` row for that account; turning it back ON re-created it. **Net zero, verified back at `value='1'`.** Also toggled "Notify on Open" '1'→'0'→'1' as the control experiment — **restored and verified**.
