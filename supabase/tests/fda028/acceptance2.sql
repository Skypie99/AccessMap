...
BEGIN
  ...
  SELECT a.decision, a.flag_id INTO d, fid
    FROM limiter.admit_guest_flag(NULL, NULL, 1, 2, 'ramp', 3, 'x', t0) a;
  ...
  SELECT a.decision, a.flag_id INTO d, fid
    FROM limiter.admit_guest_flag('not-an-ip', NULL, 1, 2, 'ramp', 3, 'x', t0) a;
  ...
  SELECT a.decision, a.out_grant, a.flag_id INTO d, gr, fid
    FROM limiter.admit_guest_flag('198.51.100.10', NULL, 10.5, 20.5, 'ramp', 3, 'desc', t0) a;
  ...
  SELECT a.decision INTO d FROM limiter.admit_guest_feedback('198.51.100.11', NULL, 'bug', 'body', 'ios', t0) a;
  PERFORM pass('feedback: admitted + row written',
    d='ADMITTED' AND (SELECT count(*) FROM public.feedback) = fb0 + 1);
  SELECT a.decision INTO d FROM limiter.admit_guest_feedback(NULL, NULL, 'bug', 'body', 'ios', t0) a;
  ...
  SELECT a.out_grant INTO g1 FROM limiter.admit_guest_flag('203.0.113.50', NULL, 1,2,'ramp',3,'a', t0) a;
  SELECT a.decision INTO d FROM limiter.admit_guest_flag('203.0.113.50', g1, 1,2,'ramp',3,'b', t0) a;
  PERFORM pass('reset: second spend admitted', d='ADMITTED');
  SELECT a.decision INTO d FROM limiter.admit_guest_flag('203.0.113.50', g1, 1,2,'ramp',3,'c', t0) a;
  PERFORM pass('reset: grant exhausted', d='REFUSED_GRANT');
  -- client discards ALL state and asks again
  SELECT a.decision, a.out_grant INTO d, g2 FROM limiter.admit_guest_flag('203.0.113.50', NULL, 1,2,'ramp',3,'d', t0) a;
  ...
  FOR n IN 1..12 LOOP
    SELECT a.decision INTO d FROM limiter.admit_guest_flag('203.0.113.50', NULL, 1,2,'ramp',3,'e', t0) a;
  ...
  SELECT a.decision INTO d FROM limiter.admit_guest_flag('203.0.113.50', NULL, 1,2,'ramp',3,'f', t0 + interval '61 seconds') a;
  PERFORM pass('reset: natural window renewal restores budget', d='ADMITTED');

  -- ===== CLIENT INDEPENDENCE =====
  SELECT a.out_grant INTO g1 FROM limiter.admit_guest_flag('203.0.113.60', NULL, 1,2,'ramp',3,'a', t0) a;
  SELECT a.out_grant INTO g2 FROM limiter.admit_guest_flag('203.0.113.60', NULL, 1,2,'ramp',3,'b', t0) a;
  PERFORM pass('independence: two grants on one bucket', g1 <> g2);
  PERFORM limiter.admit_guest_flag('203.0.113.60', g1, 1,2,'ramp',3,'c', t0);
  SELECT a.decision INTO d FROM limiter.admit_guest_flag('203.0.113.60', g1, 1,2,'ramp',3,'d', t0) a;
  PERFORM pass('independence: A exhausted', d='REFUSED_GRANT');
  SELECT a.decision INTO d FROM limiter.admit_guest_flag('203.0.113.60', g2, 1,2,'ramp',3,'e', t0) a;
  ...
  FOR n IN 1..5 LOOP
    PERFORM limiter.admit_guest_flag('203.0.113.70', NULL, 1,2,'ramp',3,'r', t0);
  END LOOP;
  SELECT a.decision INTO d FROM limiter.admit_guest_flag('203.0.113.70', NULL, 1,2,'ramp',3,'new', t0) a;
