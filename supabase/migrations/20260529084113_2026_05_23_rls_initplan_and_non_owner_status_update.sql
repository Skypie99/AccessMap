...
-- ledger (supabase_migrations.schema_migrations), version 20260529084113, hosted name "2026_05_23_rls_initplan_and_non_owner_status_update".
-- This version previously had no local managed migration file. Below is the exact
-- hosted-recorded SQL, verbatim.

-- Drop and recreate all RLS policies with (select auth.uid()) instead of bare auth.uid()
...
create policy "flags status update by any authenticated"
  ...
  using (true)
  with check (
    status != (select status from public.flags where id = flags.id)
    ...
    and description = (select description from public.flags where id = flags.id)
    ...
    and photo_url = (select photo_url from public.flags where id = flags.id)
  );
