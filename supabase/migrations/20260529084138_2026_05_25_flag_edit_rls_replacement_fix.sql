...
-- ledger (supabase_migrations.schema_migrations), version 20260529084138, hosted name "2026_05_25_flag_edit_rls_replacement_fix".
-- This version previously had no local managed migration file. Below is the exact
-- hosted-recorded SQL, verbatim.

-- Drop both old and new policy variants to ensure clean state
drop policy if exists "flags update own" on public.flags;
drop policy if exists "flags owner edit open" on public.flags;

-- Create the new policy with open status guard
